...
def semantic_search_no_change(queries, questions, model="msmarco-distilbert-base-v4"):
    sem_list = []
    ...
    for i in range(3):
        ...
        print(type(hits))
        print("hit is equal to : ", hits)
        hits = hits[0]

        cross_inp = [[query, questions[hit['corpus_id']]] for hit in hits]
        cross_scores = cross_encoder.predict(cross_inp)

        print("cross encoder scores are : ", cross_scores)
...
def main():
    ...
    sem_list = semantic_search_no_change(new_query_index, questions)
    sem_list = sem_list[:-12]
    indexes = []
    for i in range(len(sem_list)):
        indexes.append(sem_list[i][0])
    print(indexes)
    result1 = [(queries[sem_list[i][0]][2], sem_list[i][1]) for i in range(len(sem_list))]
    print(result1[:3])
    ...
    #
    # print("results without change are : ", result1[:3])
    # print("results with change are : ", result2[:3])
